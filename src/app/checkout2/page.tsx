...
import Currais2UpsellPopup from '@/components/Currais2UpsellPopup';
...
const PRODUCTS_MAP: Record<string, ProductDetails> = {
  currais2: {
    id: 'currais2',
    name: 'Pacote 75 Projetos de Currais A2',
...
function CheckoutForm2() {
  ...
  const productId = searchParams.get('p') || 'currais2';
  const product = PRODUCTS_MAP[productId] || PRODUCTS_MAP.currais2;
  ...
  const [showCurrais2UpsellPopup, setShowCurrais2UpsellPopup] = useState(false);
  const [hasShownUpsell, setHasShownUpsell] = useState(false);

  // Preço estático de upsell avulso
  const curraisAvulsoPrice = 13.90;
  ...
  if (isComboActive) {
    computedTotal += 19.80; // Combo promocional: R$ 9,90 cada (Total: R$ 39,70)
  } else {
    ...
    try {
      const response = await fetch('/api/checkout', {
        ...
        body: JSON.stringify({
          ...
          course_id: product.id, // Envia 'currais2'
        }),
      });
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    if (!arrendamentoSelected && !planilhaSelected && !hasShownUpsell) {
      setShowCurrais2UpsellPopup(true);
  ...
  return (
    <div className={styles.checkoutPage}>
      ...
      <Currais2UpsellPopup
        isOpen={showCurrais2UpsellPopup}
        onAccept={handleAcceptCurrais2Upsell}
        onDecline={handleDeclineCurrais2Upsell}
      />
...
export default function GenericCheckoutPage2() {
  return (
