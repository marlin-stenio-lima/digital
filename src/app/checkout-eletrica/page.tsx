...
import EletricaUpsellPopup from '@/components/EletricaUpsellPopup';
...
const PRODUCTS_MAP: Record<string, ProductDetails> = {
  eletrica_completa: {
    id: 'eletrica_completa',
    name: 'Projetos e Comandos Elétricos Completo',
    description: 'Curso prático com 500 esquemas e diagramas em PDF',
    originalPrice: 97.00,
    salePrice: 9.90,
  }
};
...
function CheckoutEletricaForm() {
  const searchParams = useSearchParams();
  const productId = searchParams.get('p') || 'eletrica_completa';
  const product = PRODUCTS_MAP[productId] || PRODUCTS_MAP.eletrica_completa;
  ...
  // States de Order Bumps e Upsells (PEDREIRO/PORCELANATO/CUBAS)
  const [pedreiroSelected, setPedreiroSelected] = useState(false);
  ...
  // Preços promocionais avulsos
  const pedreiroAvulsoPrice = 13.90;
  const porcelanatoAvulsoPrice = 9.90;
  const cubasAvulsoPrice = 9.90;
  ...
  // Se todos forem selecionados, aplica o combo de R$ 29,80 adicionais
  const isComboActive = pedreiroSelected && porcelanatoSelected && cubasSelected;
  if (isComboActive) {
    computedTotal += 29.80; // Total: R$ 39,70
  } else {
    if (pedreiroSelected) computedTotal += pedreiroAvulsoPrice;
    if (porcelanatoSelected) computedTotal += porcelanatoAvulsoPrice;
    if (cubasSelected) computedTotal += cubasAvulsoPrice;
  }
    ...
    try {
      const response = await fetch('/api/checkout', {
        ...
        body: JSON.stringify({
          ...
          course_id: product.id, // Envia 'eletrica_completa'
        }),
      });
  ...
  const handleSubmit = (e: FormEvent) => {
    ...
    if (!pedreiroSelected && !porcelanatoSelected && !cubasSelected && !hasShownUpsell) {
      setShowUpsellPopup(true);
      setHasShownUpsell(true);
    } else {
      const buyList = [product.id];
      if (pedreiroSelected) buyList.push('pedreiro');
  ...
  const handleAcceptUpsell = () => {
    ...
    setPedreiroSelected(true);
    setPorcelanatoSelected(true);
    setCubasSelected(true);
    
    // Combo de adicionais: R$ 29,80
    const buyList = [product.id, 'pedreiro', 'porcelanato', 'cubas'];
    generatePix(product.salePrice + 29.80, buyList);
  };
      ...
      <div className={styles.container}>
        ...
        <div className={styles.orderSummary}>
          ...
          <div className={styles.summaryList}>
            ...
            {pedreiroSelected && (
              <div className={styles.summaryRow}>
                <span>🏗️ Curso Mestre de Obra</span>
                <span>R$ {pedreiroAvulsoPrice.toFixed(2).replace('.', ',')}</span>
              </div>
            )}
            ...
            {porcelanatoSelected && (
              <div className={styles.summaryRow}>
                ...
                <span>R$ {porcelanatoAvulsoPrice.toFixed(2).replace('.', ',')}</span>
              </div>
            )}

            {cubasSelected && (
              <div className={styles.summaryRow}>
                <span>🏺 Fabricação de Cubas</span>
                <span>R$ {cubasAvulsoPrice.toFixed(2).replace('.', ',')}</span>
              </div>
            )}
          </div>
        ...
        <form className={styles.formSection} onSubmit={handleSubmit} noValidate>
          ...
          <div className={styles.orderBumpsSection}>
            ...
            {/* BUMP 1: CURSO DE PEDREIRO */}
            <label className={`${styles.bumpCard} ${pedreiroSelected ? styles.bumpSelected : ''}`}>
              <input 
                type="checkbox" 
                checked={pedreiroSelected}
                onChange={(e) => setPedreiroSelected(e.target.checked)}
                className={styles.bumpCheckbox}
              />
              <div className={styles.bumpInfo}>
                <div className={styles.bumpHeader}>
                  <span className={styles.bumpName}>🏗️ Curso Mestre de Obra & Pedreiro</span>
                  <span className={styles.bumpPrice}>+ R$ {pedreiroAvulsoPrice.toFixed(2).replace('.', ',')}</span>
                </div>
                <p className={styles.bumpDesc}>
                  Treinamento profissional completo em videoaulas estruturadas sobre alvenaria, rebocos, fundações e coberturas.
                </p>
              </div>
            </label>
            ...
            <label className={`${styles.bumpCard} ${porcelanatoSelected ? styles.bumpSelected : ''}`}>
              ...
              <div className={styles.bumpInfo}>
                <div className={styles.bumpHeader}>
                  ...
                  <span className={styles.bumpPrice}>+ R$ {porcelanatoAvulsoPrice.toFixed(2).replace('.', ',')}</span>
                </div>
                <p className={styles.bumpDesc}>
                  Manuais de medidas de bancadas, ilhas gourmet e nichos em porcelanato com desenhos estruturados em PDF.
                </p>
              </div>
            </label>
            ...
            <label className={`${styles.bumpCard} ${cubasSelected ? styles.bumpSelected : ''}`}>
              ...
              <div className={styles.bumpInfo}>
                <div className={styles.bumpHeader}>
                  ...
                  <span className={styles.bumpPrice}>+ R$ {cubasAvulsoPrice.toFixed(2).replace('.', ',')}</span>
                </div>
                <p className={styles.bumpDesc}>
                  Guia passo a passo de fôrmas, dosagem e acabamentos com efeito granito, mármore e rústico para cubas finas.
                </p>
              </div>
            </label>
          </div>
